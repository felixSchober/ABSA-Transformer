...
The easiest way to achieve this is a one-hot encoding of a vocabulary. We just take the vocabulary which is a list of $N$ words and number the words sequentially. A word vector is created by getting the index of the word from the vocabulary and setting the value of the word vector at the word-index to one.
...
In 1992 Schütze {(and to some extend Hinton in 1986 \cite{Hinton1986})} pioneered the idea of an universal word vector space \cite{Schutze1992}. The general idea is that instead of having sparse, high dimensional word encodings it would be better to have low-dimensional, dense word vectors where semantically similar words appear close together in the word vector space. Unrelated words should be far away from each other. For example, the distance between the words "dog" and "cat" should be low while the distance between "bed" and "rocket" should be high.
\medskip

To achieve this, Schütze suggested to count co-occurrence and construct a co-occurrence matrix \cite{Schutze1992}. While this makes the word-matrix dense {(only 2\% of the entries are zero)} the resulting matrix is still extremely large and several dimensionality reduction techniques had to be applied.
\medskip
...
Instead of improving $W$ directly, we use $W$ to train a network on an auxiliary task like predicting the next word in a sequence. Instead of training $W$ supervised we use an unsupervised task and train $W$ indirectly. This is the basic principle how many word embeddings are trained and interestingly, this produces fascinating results. 

\begin{figure}
	\centering
	\includegraphics[width=0.33\textwidth]{figures/03_theory/03_wordEmbeddings1}
	\includegraphics[width=0.33\textwidth]{figures/03_theory/03_wordEmbeddings2}
	\includegraphics[width=0.33\textwidth]{figures/03_theory/03_wordEmbeddings3}
...
"The use of word representations… has become a key “secret sauce” for the success of many NLP systems in recent years, across tasks including named entity recognition, part-of-speech tagging, parsing, and semantic role labeling." \cite{Luong2013}

explain why we need word representations




visualization \cite{Turian2010} http://metaoptimize.com/projects/wordreprs/
