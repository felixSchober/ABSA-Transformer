...
To achieve this, Schütze suggested to count co-occurrence of words and their neighbors and construct a co-occurrence matrix \cite{Schutze1992}. While this makes the word-matrix dense {(only 2\% of the entries are zero)} the resulting matrix is still extremely large and several dimensionality reduction techniques had to be applied.
\medskip
...
\gls{cbow} is very similar to the skip-gram model. However, instead of predicting context words for source word, \gls{cbow} predicts the next word $w_{t}$ in a sequence, given the previous words $w_{t-j}$ \cite{Mikolov2013c}. In other words, the objective of \gls{cbow} is to predict a word given it's context.
\medskip
